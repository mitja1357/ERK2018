...
After defining approximation funtions of error due to deformation parameter in input signal, a fault to (\ref{equ:def_err}) was made. Approximation functions were defined with Fourier series, during analysis of fault were used first 15 components only. Example, input signals have amplitudes $A_1 = 1$ and $B_1 = 1.1$. Difference between calculated error in (\ref{equ:amp_err2}) and error in (\ref{equ:def_err})  is presented in figure \ref{fig:razlika}. Fault of approximation is in the vicinity of a numeric error. Comparing errors in frequency specter, showed minimum deviations between harmonics of same frequency.

It is necessary to mention that despite the derivation, the presented types of error of individual deformations are still depend on one other.
\begin{figure}[!htb]
	\begin{center}
