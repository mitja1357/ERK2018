...
\end{eqnarray}
Definition of output from atan2 $\varphi$ is:
\begin{eqnarray}
\label{equ:def_kot}
...
\end{equation}
By varying each parameter from eq. (\ref{equ:def_sin})(\ref{equ:def_cos})(\ref{equ:def_eks_sin})(\ref{equ:def_eks_cos}) individually and calculating its FFT, we get numeric values of amplitudes and phases  for first n harmonics.
Sledi fitanje poteka posameznega harmonika na poljubno funkcijo, ki bo čim bolje opisala potek amplitude na posamezen parameter iz (1234).
