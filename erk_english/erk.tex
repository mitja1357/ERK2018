...
Input signals can also contain same DC component. This happen to resolver signals acquired at improper installation of stator coils. Same analysis was made and result is presented in eq. (\ref{equ:offsc_err}).
\begin{multline}
\label{equ:offsc_err}
...
Added common mode signal to input signals was analyzed for each parameter individually. This paper presents procedure for CCM of cosinu signal only. Result of CCM sinusoidal form is appended. Procedure to achieve result was the same.
...
\begin{multline}
\label{equ:dc_err}
\varepsilon(A_1, \Delta_c) = \mathrm{atan}\frac{\Delta_c}{\Delta_c+2 A_1}+\\+\frac{180}{\pi} \sum_{n=1}^{\infty}\frac{1}{n} (\frac{\Delta_c}{\sqrt{\Delta_c^2+2 A_1 \Delta_c+2 A_1}})^n\\ \sin (2n \theta+n (90^\circ+ \mathrm{ atan}(\frac{\Delta_c+A_1}{A_1})))
\end{multline}

Same procedure was made for $\Delta_s$. Result is in eq. (\ref{equ:ds_err})
\begin{multline}
\label{equ:ds_err}
\varepsilon(A_1, \Delta_s) = \mathrm{atand}\frac{-\Delta_s}{\Delta_s+2 A_1}+\\+\frac{180}{\pi} \sum_{n=1}^{\infty}\frac{1}{n} (\frac{\Delta_s}{\sqrt{\Delta_s^2+2 A_1 \Delta_s+2A_1}})^n\\ \sin (2n \theta+n (90^\circ+ \mathrm{ atan}(\frac{\Delta_s+A_1}{A_1})))
\end{multline}
...
After defining error due to deformation parameter in input signal, comparison between error in (\ref{equ:def_err}) and approximated functions defined in this paper was made. Approximated functions were defined with infinity series, during comparison were used first 15 components only. In example input signals have amplitudes $A_1 = 1$ and $B_1 = 1.1$. Difference between error calculated with (\ref{equ:amp_err2}) and error from (\ref{equ:def_err})  is presented in figure \ref{fig:razlika}. Difference between subtracted error is in numerical range. Comparing those two errors in frequency specter, showed minimum deviations between harmonics of same frequency.
...
This paper examines the impact of improper installation of magnetic encoders and resolvers on angle error. With methods described in this paper, improper installation of encoders and resolvers can be uncovered from analyzing error only. This paper presented content of higher harmonics in error, which become non-negligible at major mismatches. This paper present impact of signal mismatch, non-orthogonality, DC offset and common mode signal to output error of atan2 function.
