...
By varying the parameter k from 0 to infinity, we found that the error specter consists of even harmonics only. We also observed that phase shift didn't change. When k approaches infinity, the amplitude of harmonics approaches $\frac{180}{\pi}\frac{2}{n}$, where n represents n-th harmonic. Eq. (\ref{equ:lim_amp_vrsta}) contains even harmonics only.
\begin{equation}
\label{equ:lim_amp_vrsta}
\varepsilon(k \rightarrow \infty) = \frac{180}{\pi}\sum_{n=1}^{\infty}\frac{1}{n} \sin 2 n \theta
\end{equation}
Because of highest amplitude of second harmonic, function that best approximate waveform was searched first (Figure \ref{fig:amp}).
...
Amplitudes was fixed to one, eq. (\ref{equ:def_sin}) and (\ref{equ:def_cos}) were depended to phases only. Input signals are presented as:
\begin{eqnarray}
...
\begin{figure}[!htb]
	\begin{center}
		...
		\caption{The waveforms of DC component $C_0$, amplitude of second harmonic  $C_2$ and phase of error $\varphi_2$ depend to ideal cosine signal,due to phase shift $\varphi_{s}$} \label{fig:fis}
	\end{center}
\end{figure}
...
Considering, changing parameters of DC components in (\ref{equ:def_sin}) and (\ref{equ:def_cos}) only, present input signals as:
\begin{eqnarray}
&Sin = sin(\theta)+ B_0\\
&Cos = cos(\theta) + A_0.
\end{eqnarray}
First was analyzed offset in $Cos$ signal. 
\begin{equation}
...
Signals from encoder or resolver can contain equal DC component. 
\begin{multline}
\label{equ:offsc_err}
