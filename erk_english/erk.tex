...
These days the need for high quality motor regulation is present in numerous applications and has as a result become unavoidable. For a consistent and reliable measurement of rotation, position sensors are used \cite{uporaba_senzorjev}, such as  encoders and resolvers \cite{inkrementalni}\cite{resolver}\cite{enkoder}. However, the output of which is a pair of quadrature sine and cosine signals. Angle information must first be calculated. The easiest way of doing so is by directly calculating with atan2 that returns a value between $[-\pi, \pi]$ \cite{atan}.

Quadrature sine and cosine signals have not perfect form of mathematical functions sine and cosine. Quadrature sine and cosine signals can contain  non-equal amplitudes, offsets or phase shifts. Despite non-perfect quadrature signals, angle is still calculated. Because of incorrect signal inputs of function atan2, calculated angel includes error.

Literature  \cite{RLS1}, \cite{RLS2}, \cite{RLS3} analyzed the effect of incorrect input signals of atan2 function. However, angel error has been presented by most outstanding harmonic only. The course of error has been presented  by linear function only. Results of literature are valid for relatively small incorrectness of input signals only. During  research was noticed that error contains  higher harmonics too. This paper presents angle error by infinite series. Amplitude and phase of harmonics depends on parameter, that describes incorrect quadrature sine and cosine signals which are inputs of atan2.

\section{Methodology and results}
Iinput signals to atan2 have form of sine and cosine. Nevertheless  input signals could contain different amplitude, offsets or phase shifts(\ref{equ:def_sin})(\ref{equ:def_cos})
\begin{eqnarray}
\label{equ:def_sin}
...
\end{eqnarray}
Ideal case is when offsets ($B_0$ and $A_0$) and phase shifts $\varphi_s$ and $\varphi_c$  are equal to zero. In ideal case amplitudes are same ($B_1 = A_1$).
In this case error is zero. This research analyzed, course of error for each parmeter of (\ref{equ:def_sin}) and (\ref{equ:def_cos}) seperatly.
Amplitude and phase can be depended on one parameter only. To input signals can be added signal with same frequency ($\Delta_c$ or $\Delta_s$) (\ref{equ:def_eks_sin})(\ref{equ:def_eks_cos}). Research was done for $\Delta_s$ and $\Delta_c$ separately too. 
...
\end{eqnarray}
where $\theta$ presents reference angle. In MATLAB is defined atan2d(), function for calculation of inverse tangent in four quadrant plane. Output is between $[-180^\circ,180^\circ]$\cite{atand}. Error is presented by infinite series of harmonics (\ref{equ:potek_napake}).
\begin{equation}
...
Error is determinate with following approach. Limit of error is made of one parameter from (\ref{equ:def_sin}), (\ref{equ:def_cos}), (\ref{equ:def_eks_sin}), (\ref{equ:def_eks_cos}) to infinity or to the worst case. Output of atan2d $\varphi$ become constant, error $\varepsilon$  could be expressed as is described in (\ref{equ:def_err_inf}).
\begin{equation}
